...
export default ( { props, post } ) => {
    ...
    const Excerpt = () => {
        ...
        switch ( excerptType ) {
            case 'excerpt':
                excerpt                     = ((post.excerpt !== null && post.excerpt !== undefined) && post.excerpt.raw.length > 0 ) ? post.excerpt.raw : post.content.raw;
    ...
    const MetaCategories = () => {
        if ( post.categories === null || post.categories === undefined || post.categories.length < 1 || ! categories || ! showCategories ) return null;
       ...
       return (
        <div className="sv-posts-categories">
            <i className={ infoIconColorClass } style={{ color: infoIconColor }}>{ iconArchive }</i>
            { output }
        </div>
       );
    };
